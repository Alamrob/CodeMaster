...
def test_scan_evidence_filter(capsys, tmp_path: Path) -> None:
    ...
    code = main(["scan", "--json", "--evidence", "strong", str(tmp_path)])
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
    names = [f["path"] for f in payload["files"]]
    assert any(n.endswith("strong.py") for n in names)
    assert not any(n.endswith("weak.md") for n in names)
